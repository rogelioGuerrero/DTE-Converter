...
import { Search, Package, DollarSign, AlertCircle, TrendingUp, Star } from 'lucide-react';
import { Producto, ProductoFactura } from '../../types/inventario';
...
const SelectorProductosFactura: React.FC<SelectorProductosFacturaProps> = ({
  ...
  useEffect(() => {
    if (busqueda.length >= 2) {
      ...
      const productosDisponibles = resultadosBusqueda.filter(producto => 
        !productosExistentes.some(pe => pe.producto.id === producto.id)
      );
  ...
  const handleAgregarProducto = () => {
    ...
    const productoFactura: ProductoFactura = {
      ...
      cantidad,
      precioUnitario,
      loteSeleccionado: undefined // El sistema seleccionará automáticamente
    };
          ...
          <div className="flex flex-wrap gap-2">
            {favoritos.map(producto => (
              <button
                ...
                className="flex items-center gap-1 px-3 py-1 bg-yellow-50 hover:bg-yellow-100 text-yellow-800 rounded-full text-sm transition-colors"
              >
