...
import { X, Save, DollarSign, ToggleLeft, ToggleRight } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (producto) {
        ...
        producto.descripcion = formData.descripcion;
        producto.categoria = formData.categoria;
        producto.codigo = formData.codigo;
        producto.precioSugerido = precioSugerido;
        producto.gestionarInventario = formData.gestionarInventario;
        producto.esFavorito = formData.esFavorito;
        producto.unidadBase = (formData.unidadBase || 'UNIDAD').toUpperCase();
        producto.presentaciones = presentaciones.map((p) => ({ nombre: (p.nombre || '').toUpperCase(), factor: Number(p.factor) || 1 }));
        producto.presentacionesPendientes = presentacionesPendientes;
        ...
        productoGuardado.precioSugerido = precioSugerido;
        productoGuardado.gestionarInventario = formData.gestionarInventario;
        productoGuardado.esFavorito = formData.esFavorito;
        productoGuardado.unidadBase = (formData.unidadBase || 'UNIDAD').toUpperCase();
        productoGuardado.presentaciones = presentaciones.map((p) => ({ nombre: (p.nombre || '').toUpperCase(), factor: Number(p.factor) || 1 }));
        productoGuardado.presentacionesPendientes = presentacionesPendientes;
        inventarioService.actualizarProducto(productoGuardado);
      }
